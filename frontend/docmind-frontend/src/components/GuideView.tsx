...
import {
  ...
  ShieldCheck, RefreshCw, GitCompare, Clock, Network, Image as ImageIcon,
...
const GuideView: React.FC<GuideViewProps> = ({ onUploadClick }) => {
  ...
  const capabilities = [
    {
      ...
      tag: "KNOWLEDGE VISUALIZATION",
      tagColor: "bg-teal-500/10 text-teal-400 border-teal-500/20",
      title: "Interactive Mind Map & Concept Hierarchy",
      desc: "Extracts entities, core topics, and sub-concepts from document vectors and generates an interactive, draggable node tree graph with pan/zoom, node collapse/expand, 1-click Ask AI queries, and multi-format download (PNG, SVG, MD)."
    },
    {
      icon: <ImageIcon className="w-5 h-5 text-cyan-400" />,
      tag: "MULTI-MODAL RAG",
      tagColor: "bg-cyan-500/10 text-cyan-400 border-cyan-500/20",
      title: "Multi-Modal Image & Architecture Diagram Citations",
      desc: "Extracts embedded architecture diagrams, workflow charts, and system design images from PDFs during ingestion using Apache PDFBox 3.x. Renders visual diagram preview cards inline inside chat with full-screen interactive zoom/pan Lightbox."
    },
    {
      icon: <GraduationCap className="w-5 h-5 text-purple-400" />,
      tag: "ACTIVE RECALL & STUDY",
      tagColor: "bg-purple-500/10 text-purple-400 border-purple-500/20",
      title: "Interactive AI Quiz & Flashcard Study Decks",
      desc: "Generates interactive multiple-choice quizzes and 3D flip flashcard decks from your document chunks. Features instant grading, score percentages, and verified source explanations."
    },
    {
  ...
  const steps = [
    {
      num: "01",
      title: "Upload Documents & Extract Diagrams",
      desc: "Upload PDF, DOCX, TXT, MD, or CSV files. Mindora parses text, splits content into 700-character chunks, indexes embeddings in pgvector, and automatically extracts high-resolution architecture diagrams via Apache PDFBox 3.x.",
      action: onUploadClick ? { label: "Upload Document", onClick: onUploadClick, icon: Upload } : undefined
    },
    {
      num: "02",
      title: "Visualize with Interactive Mind Map",
      desc: "Switch to Mind Map view to generate a structured concept tree. Drag, zoom, expand sub-branches, inspect topic summaries, and ask the AI specific concept questions in 1 click.",
      action: { label: "Open Mind Map", onClick: () => setActiveTab('mindmap'), icon: Network }
    },
    {
      num: "03",
      title: "Chat with Multi-Modal Citations",
      desc: "Ask questions via voice or text. Mindora streams answers with token telemetry, verified page citations, and interactive Lightbox architecture diagram previews.",
      action: { label: "Open Chat Assistant", onClick: () => setActiveTab('chat'), icon: MessageSquare }
    },
    {
      num: "04",
      title: "Test Knowledge in Study Hub",
      desc: "Generate active recall AI quizzes and 3D flashcards directly from your document chunks to master complex architectures and documentation.",
      action: { label: "Open Quiz & Study Hub", onClick: () => setActiveTab('study'), icon: GraduationCap }
    }
  ];
          ...
          <div className="relative z-10 space-y-5">
            ...
            <div className="flex items-center gap-3 pt-2 flex-wrap">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-teal-500/10 hover:bg-teal-500/20 border border-teal-500/30 text-teal-600 dark:text-teal-300 text-xs sm:text-sm font-medium transition-all shadow-sm"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/30 text-purple-600 dark:text-purple-300 text-xs sm:text-sm font-medium transition-all shadow-sm"
              >
                <GraduationCap className="w-4 h-4 text-purple-400" />
                <span>Quiz & Study Hub</span>
              </button>
            ...
            <div className="flex items-center gap-2 pt-4 border-t border-slate-200 dark:border-slate-800/80 flex-wrap text-[11px] text-slate-600 dark:text-slate-400">
              ...
              <span className="flex items-center gap-1.5 bg-white dark:bg-slate-900/90 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <span>Multi-Modal PDFBox Citations</span>
              </span>
              <span className="flex items-center gap-1.5 bg-white dark:bg-slate-900/90 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-800 shadow-sm">
