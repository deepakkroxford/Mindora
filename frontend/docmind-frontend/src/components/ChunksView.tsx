...
const ChunkCard: React.FC<{ chunk: CitationDto; index: number }> = ({ chunk, index }) => {
  ...
  return (
    <div
      className={clsx(
        'border rounded-2xl bg-slate-900/60 overflow-hidden group transition-all duration-200',
        expanded ? 'border-teal-500/50 shadow-md shadow-teal-500/10' : 'border-slate-800/80 hover:border-slate-700'
      )}
    >
      {/* Chunk Header */}
      <div
      ...
      >
        {/* Index badge */}
        <div className="flex-shrink-0 w-8 h-8 bg-teal-500/15 border border-teal-500/25 rounded-xl flex items-center justify-center">
          <span className="text-xs font-bold text-teal-400 font-mono">#{chunk.chunkIndex ?? index}</span>
        </div>

        {/* File name & snippet preview */}
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-white truncate">{chunk.fileName}</p>
          <div className="flex items-center gap-2 mt-0.5">
            {chunk.pageNumber != null && (
              <span className="text-[11px] text-teal-400/90 flex items-center gap-0.5 bg-teal-500/10 px-1.5 py-0.2 rounded font-mono">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {pct != null && (
            <span className="text-xs bg-teal-500/10 text-teal-300 border border-teal-500/25 px-2 py-0.5 rounded-full font-mono font-semibold">
              {pct}%
            </span>
          )}
          <button
            onClick={(e) => { e.stopPropagation(); handleCopy(); }}
            className="p-1 text-slate-500 hover:text-white hover:bg-slate-800 transition-all rounded-lg opacity-0 group-hover:opacity-100"
      ...
      {/* Expanded Details */}
      {expanded && (
        <div className="border-t border-slate-800/80 bg-[#070b14]/70 animate-fade-in p-4 space-y-3">
          {/* Full text snippet */}
          <div className="relative bg-slate-950/80 border border-slate-800/80 rounded-xl p-4">
            <p className="text-xs text-slate-200 leading-relaxed whitespace-pre-wrap font-mono">{chunk.snippet}</p>
            <button
              onClick={handleCopy}
              className="absolute top-2.5 right-2.5 p-1.5 bg-slate-800/80 hover:bg-slate-700 border border-slate-700 rounded-lg text-slate-400 hover:text-white transition-colors"
              title="Copy snippet"
            >
              {copied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
            </button>
          ...
          {chunk.metadata && Object.keys(chunk.metadata).length > 0 && (
            <div>
              <p className="text-[11px] text-slate-400 mb-1.5 flex items-center gap-1 font-medium uppercase tracking-wider">
                <Hash className="w-3 h-3 text-teal-400" /> Metadata Properties
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                {Object.entries(chunk.metadata)
                  .filter(([k]) => !['snippet'].includes(k))
                  .map(([k, v]) => (
                    <div key={k} className="bg-slate-900/80 border border-slate-800/80 rounded-xl p-2.5">
        ...
        <div className="max-w-4xl mx-auto space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-sm font-bold text-white">Vector Chunks Explorer</h2>
                <p className="text-[11px] text-slate-500">Inspect chunk embeddings stored in pgvector database</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              {hasLoaded && (
                <div className="flex items-center gap-1.5 text-xs text-slate-400 font-mono">
                  <span className="bg-slate-900 border border-slate-800 px-2 py-0.5 rounded-lg text-teal-300">
              ...
              <button
                onClick={() => loadChunks()}
                disabled={isLoading}
                className="flex items-center gap-1.5 text-xs border border-slate-800 hover:border-slate-700 bg-slate-900/60 text-slate-400 hover:text-white px-3 py-1.5 rounded-xl transition-colors disabled:opacity-40"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div className="flex items-center gap-2 bg-slate-900/80 border border-slate-700/60 rounded-xl px-3 py-2 focus-within:border-teal-500/50 transition-colors">
              ...
              <button
                onClick={() => loadChunks()}
                disabled={isLoading}
                className="text-[10px] font-semibold text-teal-400 hover:text-teal-300 uppercase tracking-wider"
              >
                Query
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 bg-slate-900/80 border border-slate-700/60 rounded-xl px-3 py-2 focus-within:border-teal-500/50 transition-colors">
              ...
              />
              {textFilter && (
                <button onClick={() => setTextFilter('')} className="text-slate-500 hover:text-slate-300">
        ...
        <div className="max-w-4xl mx-auto">
          {!hasLoaded && !isLoading && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="text-lg font-bold text-slate-200 mb-1">Inspect pgvector chunks</h3>
          ...
          {isLoading && (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              ...
              <div className="text-center">
                <p className="text-slate-200 font-medium">Loading vector records…</p>
          ...
          {hasLoaded && !isLoading && filtered.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-slate-300 font-medium">{textFilter ? 'No chunks match keyword filter' : 'No vector chunks found'}</p>
